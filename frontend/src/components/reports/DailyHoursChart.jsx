...
export default function DailyHoursChart({ data, startDate, endDate }) {
  ...
  // Extract unique clients from data (if any)
  ...
  // Transform data for daily totals chart - include all dates in range
  ...
  // Transform data for by-client stacked chart - include all dates in range
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chart 2: Daily Hours by Client (Stacked Bar) */}
      <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {byClientData.map((row) => {
                const dailyTotal = clients.reduce((sum, client) => sum + (row[client] || 0), 0)
                return (
                  <tr key={row.date}>
                    ...
                    {clients.map(client => (
                      <td key={client} className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 text-right">
                        {row[client] ? row[client].toFixed(2) : '-'}
                      </td>
                    ))}
