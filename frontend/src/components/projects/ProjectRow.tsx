...
export default function ProjectRow({ project, onEdit, onArchive, onRestore, onDelete }: ProjectRowProps) {
  const budgetPercentage = project.hour_budget
    ? ((project.hours_logged || 0) / project.hour_budget) * 100
    : null
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-md hover:shadow-lg transition-all">
        ...
        <div className="flex-1">
          ...
          <p className="text-base text-gray-500 mt-0.5">
            ...
            {project.hourly_rate_override && (
              <span className="text-sm text-blue-600 ml-1">(custom rate)</span>
            )}
          </p>
          ...
          {project.hour_budget ? (
            <div className="mt-2">
              <div className="flex justify-between text-base text-gray-600 mb-1">
                <span>Budget</span>
                <span>
                  {formatHours(project.hours_logged || 0)} / {formatHours(project.hour_budget)}
            ...
            <p className="text-base text-gray-500 mt-1">
              {formatHours(project.hours_logged || 0)} logged · No budget set
            </p>
          )}
        </div>
